...
概念刚度和正则速度没有经过轮胎试验拟合，不能作为轮胎真实性结论。V0.9 已提供稳态代数高级轮胎适配契约，可替换该本构而不改变 QSS core；未经来源、schema、坐标、轮胎侧和有效域验证的外部模型仍不会启用。
...
- 当前 ODE 接口没有 aero，因此只允许与 aero-off、未校准的理论 GGV 对比；GGV 必须携带相同 vehicle、envelope-relevant tire、powertrain、brake provenance 和标准重力，事件未到达时不进行比较。
...
使用 `simple_saturated` 时，GGV 与 7DOF 共用轮胎包络，因此对比只属于实现一致性检查。使用 `external_adapter` 时，对比变为独立稳态本构与 QSS envelope 的交叉检查；它能暴露模型差异，但仍不能替代轮胎试验或实车验证。独立测试另检查零输入平衡、左右镜像、低速有限性、机械能符号，以及关闭载荷转移后的 RWD 解析加速范围。

当前仍未包含轮胎松弛长度、回正力矩、动态有效半径、差速器、Ackermann、滚阻、气动、悬架垂向平衡和全赛道驾驶员模型。
